  ...
  <div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="addJurusanForm" method="POST" action="{{ url('admin/sekolah-jurusan') }}" enctype="multipart/form-data">
          @csrf
          ...
          <div class="form-group">
            ...
            <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" placeholder="Masukkan Deskripsi" rows="5">{{ old('deskripsi') }}</textarea>
            @error('deskripsi')
              <span class="invalid-feedback" role="alert">{{ $message }}</span>
            @enderror
          </div>          
...
<div class="modal fade" id="editJurusanModal-{{ $j->id }}" tabindex="-1" role="dialog" aria-labelledby="editJurusanModal-label-{{ $j->id }}" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editJurusanForm-{{ $j->id }}" method="POST" action="{{ url('/admin/sekolah-jurusan/'. $j->id) }}" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          <div class="form-group">
            <label for="name">Nama Jurusan</label>
          ...
          <div class="form-group">
            ...
            <textarea class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="Masukkan Nama" rows="5">{{ old('name', $j->deskripsi) }}</textarea>
            @error('name')
              <span class="invalid-feedback" role="alert">{{ $message }}</span>
            @enderror
          </div>
...
<div class="modal fade" id="deleteJurusanModal-{{ $j->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteJurusanModal-label-{{ $j->id }}" aria-hidden="true">
  <div class="modal-dialog" role="document">
      <div class="modal-content">
          ...
          <div class="modal-footer">
              <form method="POST" action="{{ url('/admin/sekolah-jurusan/'.$j->id)}}">
                  @csrf
                  @method('DELETE')
                  <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal"><i class="fa-solid fa-close"></i></button>
